/**
 * Author: Trullify
 * Author URL: https://trullify.com
 */
"use strict";

// Core Options
const trullify_config = {
    // Smooth Scroll
    smoothScroll: false,
    ...
    // Cursor Following
    cursorFollow: '.trullify-socials-list a, .trullify-gallery-nav',

    // Links Exception
    linksException: [
        '.comment-reply-link',
        '.shadowcore-lightbox-link',
        '.trullify-lightbox-link',
        '.trullify-noanim-link'
    ]
}
...
jQuery(function ($) {
    ...
    const core = new ST_Core( trullify_config );
    ...
    function MainMenuInit() {
        template.menu = new trullify_Menu( core );
    }

    // --- PHOTOSWITE LIGHTBOX --- //
    function initPSWP() {
        if ( jQuery('.trullify-lightbox-link').length ) {
            if ( typeof PhotoSwipe === 'function' ) {
                        ...
                        <div class="pswp__scroll-wrap">\
                            ...
                            <div class="pswp__ui pswp__ui--hidden">\
                                <div class="pswp__top-bar">\
                                    ...
                                    \
                                    <button class="pswp__button pswp__button--close trullify-pswp-close" title="Close (Esc)">\
                                        <i></i>\
                                    </button>\
                                    \
                                ...
                                \
                                <button class="pswp__button trullify-pswp-nav trullify-pswp-prev pswp__button--arrow--left" title="Previous (arrow left)"></button>\
                                <button class="pswp__button trullify-pswp-nav trullify-pswp-next pswp__button--arrow--right" title="Next (arrow right)"></button>\
                                \
                ...
                // Build PSWP Gallery
                jQuery('.trullify-lightbox-link').each(function() {
                    ...
                    if ( $this.attr('data-type') && $this.attr( 'data-type' ) !== 'image' ) {
                        ...
                        if ( 'video' === $this.attr('data-type') ) {
                            this_item.html = '\
                            <div class="trullify-pswp-media--video" data-src="' + $this.attr('href') + '">\
                            ...
                            this_item.html = '\
                            <div class="trullify-pswp-media--iframe" data-player-src="'+ this_link +'" data-src="' + $this.attr('href') + '" data-type="' + this_type + '">\
                ...
                core.$dom.body.on('click', '.trullify-pswp-image-wrap', function(e) {
                    e.preventDefault();
                    e.stopPropagation();
                });
                ...
                jQuery(document).on('click', '.trullify-lightbox-link', function(e) {
                    e.preventDefault();
                    if (typeof PhotoSwipe === 'function') {
                        let $this = jQuery(this),
                        ...
                        template.pswp.lightbox.listen('resize', function() {
                            if ( jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                            if ( jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                            }
                        });
                        ...
                        template.pswp.lightbox.listen('beforeChange', function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--video').length ) {
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--video').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--video').each(function() {
                            ...
                            if ( jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--iframe').length ) {
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--iframe').width(template.pswp.resizeVideo().w).height(template.pswp.resizeVideo().h);
                                jQuery(template.pswp.lightbox.container).find('.trullify-pswp-media--iframe').each(function() {
                ...
                console.error('trullify Error: PSWP script not found.');
            }
        }
    }
    ...
    function HeroSections() {
        // Masked Blocks
        if ( jQuery('.trullify-masked-block').length ) {
            jQuery('.trullify-masked-block').each(function() {
                new trullify_Masked(this);
            });
        }
    }
    ...
    function BlocksHighlight() {
        if ( jQuery('.trullify-backlight').length ) {
            template.cursor_pos = {
                x: 0,
                y: 0
            }
            jQuery(document).on('mousemove', function(e) {
                ...
                jQuery('.trullify-backlight').css({
                    '--mouse-x': `${template.cursor_pos.x}px`,
                    '--mouse-y': `${template.cursor_pos.y}px`,
                });
            });

            if ( jQuery('.trullify-block').length ) {
                jQuery('.trullify-block').each(function() {
    ...
    // --- MARQUEE ELEMENT --- //
    function Marquee() {
        if ( jQuery('.trullify-marquee').length ) {
            jQuery('.trullify-marquee').each(function() {
                new trullify_Marquee(this);
            });
        }
    }
        ...
        if ( jQuery('.swiper').length ) {
            ...
            const init_swiper = function() {
                ...
                jQuery('.swiper').each(function() {
                    ...
                    if ( core.$dom.body.hasClass('trullify-fullscreen-page') ) {
                        options.mousewheel = true;
                    }
                    // CAROUSEL
                    if ( $this.hasClass('trullify-carousel') ) {
                    ...
                    // SLIDER
                    if ( $this.hasClass('trullify-slider') ) {
                        ...
                        // Slider Content
                        if ( $this.find('.trullify-slide-content').length ) {
                            ...
                            $this.children('.swiper-wrapper').children('.swiper-slide').each(function(i) {
                                ...
                                if ( $slide.children('.trullify-slide-content').length ) {
                                    const $slide_content = $slide.children('.trullify-slide-content');
                                    ...
                                    $media.wrap('<div class="trullify-slider-media"/>');
                                    ...
                                    $slide.children('.trullify-slider-media').css('clip-path', `url(#${(content_mask_id + i)})`);
                    ...
                    // Pagination
                    if ( $this.children('.trullify-dots').length ) {
                        options.pagination = {
                            el: '.trullify-dots',
                            dynamicBullets: true,
                    ...
                    // Slider Navigation
                    if ( $this.parent().children('.trullify-slider-nav').length ) {
                        let $this_nav = $this.parent().children('.trullify-slider-nav');
                        options.navigation = {
                            prevEl: $this_nav.children('.trullify-slider-prev')[0],
                            nextEl: $this_nav.children('.trullify-slider-next')[0],
                        }
            ...
            if ( typeof Swiper === 'function' ) {
                ...
                console.error('trullify Error: Swiper Library not found.')
            }
        }
    }
    ...
    function CircleLinks() {
        if ( jQuery('.trullify-circle-path-link:not(.is-init)').length ) {
            jQuery('.trullify-circle-path-link:not(.is-init)').each(function() {
                let $this = jQuery(this),
    ...
    function LinksEvents() {
        jQuery('.trullify-back-to-listing').on('click', function() {
            window.localStorage.setItem('trullify_return_from', this.getAttribute('data-id'));
        });
        if ( jQuery('.trullify-albums-listing').length && window.localStorage.getItem('trullify_return_from') != false ) {
            jQuery(window).trigger( 'trullify_set_album', window.localStorage.getItem('trullify_return_from') );
            window.localStorage.setItem('trullify_return_from', 0);
        } else {
            window.localStorage.setItem('trullify_return_from', 0);
        }
    }
    ...
    function InfiniteList() {
        if ( jQuery('.trullify-infinite-list-wrap:not(.is-init)').length ) {
            jQuery('.trullify-infinite-list-wrap:not(.is-init)').each(function() {
                this.classList.add('is-init');
                core.updateRequired.push( new trullify_Infinite_List(this) );
            })
        }
    }
    ...
    // --- COUNTERS --- //
    function Counters() {
        if ( jQuery('.trullify-counter:not(.is-init)').length ) {
            jQuery('.trullify-counter:not(.is-init)').each(function() {
                let $this = jQuery(this);
                this.classList.add('is-init');
                this.counter_count = function() {
                    let $counter = $this.children('.trullify-counter-number');
    ...
    function ListWithPreview() {
        if ( jQuery('.trullify-list-with-preview').length ) {
            jQuery('.trullify-list-with-preview').each(function() {
                new trullify_LWP(this);
            });
        }
    }
    ...
    function BeforeAfter() {
        if ( jQuery('.trullify-before-after:not(.is-init)').length ) {
            jQuery('.trullify-before-after:not(.is-init)').each(function() {
                jQuery(this).addClass('is-init');
                core.updateRequired.push( new trullify_Before_After(jQuery(this)) );
            });
        }
    }
    ...
    // --- TOGGLES --- //
    function Toggles() {
        if ( jQuery('.trullify-toggles-item:not(.is-init)').length ) {
            jQuery('.trullify-toggles-item:not(.is-init)').each(function() {
                let $this = jQuery(this);
                $this.addClass('is-init');
                $this.children('.trullify-toggles-item--title').append('<i class="trullify-icon trullify-icon-arrow-down"/>');
                $this.children('.trullify-toggles-item--content').slideUp(1);
                $this.on('click', function() {
                    jQuery(this).toggleClass('is-active');
                    $this.children('.trullify-toggles-item--content').slideToggle(300);
                });
                $this.on('click', '.trullify-toggles-item--content', function(e) {
                    e.stopPropagation();
                    e.preventDefault();
                });
            });
    ...
    function ScrollTrigger() {
        ...
        if ( jQuery('.trullify-inline-ribbon:not(.is-init)').length ) {
            jQuery('.trullify-inline-ribbon:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Sticky Elements
        if ( jQuery('.trullify-sticky-block:not(.is-init)').length ) {
            jQuery('.trullify-sticky-block:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Expand on Scroll
        if ( jQuery('.trullify-expand-on-scroll:not(.is-init)').length ) {
            jQuery('.trullify-expand-on-scroll:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Text Mask Fill
        if ( jQuery('.trullify-text-fill-mask:not(.is-init)').length ) {
            jQuery('.trullify-text-fill-mask:not(.is-init)').each(function() {
                if ( core.sap !== null ) {
                    let $this = jQuery(this);
                    $this.addClass('is-init');
        ...
        // Parallax Media
        if ( jQuery('.trullify-parallax-media:not(.is-init)').length ) {
            jQuery('.trullify-parallax-media:not(.is-init)').each(function() {
                let $this = jQuery(this);
    ...
    function ContactForm() {
        if ( jQuery('.trullify-contact-form').length ) {
            // The code related to the contact form is in the file: js/contact_form.js
            if ( typeof trullify_Contact_Form === 'function' ) {
                trullify_Contact_Form();
            } else {
                console.error('trullify Error: Contact form script not found.')
            }
        }
    }
    ...
    function InitAnimation() {
        ...
        template.checkURL = function( el ) {
            ...
            if ( this_href.indexOf('javascript') === 0 ) {
                ...
                jQuery(trullify_config.linksException).each(function() {
                    if ( $el.is(this) ) {
                        result = false;
                    }
                });
    ...
    // Navigate to the appropriate URL with the hash
    window.location.href = this.href;
} else if ( this.pathname === window.location.pathname && this.protocol === window.location.protocol && this.host === window.location.host ) {
                    e.preventDefault();
    ...
    function Layout() {
        ...
        if ( core.$dom.body.hasClass('trullify-fullscreen-page') ) {
            ...
            core.$dom.main.removeClass('trullify-min-content');
            ...
            if ( core.$dom.main.height() < minHeight ) {
                core.$dom.main.addClass('trullify-min-content');
    ...
    $(document).ready(function() {
        // General UI
        RightClickProtection( '.trullify-rcp-wrap' );
        MainMenuInit();
        CircleLinks();
