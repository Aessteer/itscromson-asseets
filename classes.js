...
class trullify_Before_After {
	constructor( $obj = null ) {
        ...
        this.$el = {
            $wrap: $obj,
            $before : jQuery('<div class="trullify-before-after-img trullify-before-img"/>').appendTo($obj),
            $after : jQuery('<div class="trullify-before-after-img trullify-after-img-wrap"/>').appendTo($obj),
            $divider : jQuery('<div class="trullify-before-after-divider">\
        ...
        this.$el.$after.children('img').wrap('<div class="trullify-after-img"/>');
        this.$el.$after.children('.trullify-after-img').css('background-image', 'url('+ this.$el.$wrap.attr('data-img-after') +')');
	...
	layout() {
		this.offset = this.$el.$wrap.offset().left;
		this.size = this.$el.$wrap.width();
		this.$el.$after.children('.trullify-after-img').width( this.$el.$wrap.width() ).height( this.$el.$wrap.height() );
	}
	reset() {
		this.current = 50;
		this.target = 50;
...
const baw_axis = new trullify_PanAxis( 10 );
...
class trullify_Menu {
    constructor( _core = null ) {
        ...
        this.core.$dom.header.append('<span class="trullify-active-menu-ind"></span>');
        ...
        // Create Mobile Menu
        this.mobile_menu = new trullify_Mobile_Menu( this.$nav );
    }
    layout() {
        const _self = this;
        let active_pos = '-100%';
...
class trullify_Mobile_Menu {
    constructor( $obj )  {
        ...
        this.$nav = jQuery('<nav class="trullify-mobile-nav"/>').appendTo(this.$body);
        if ( this.$el.parents('header').hasClass('is-frosted') ) {
            this.$nav.addClass('is-frosted');
        }
        this.$menu = jQuery('<ul class="trullify-mobile-menu"/>').appendTo(this.$nav);
        this.$el.children('ul').children().each(function() {
            jQuery(this).clone().appendTo(_self.$menu);
        });
        this.$overlay = jQuery('<div class="trullify-mobile-menu-overlay"/>').appendTo(this.$body);
        ...
        // Events
        jQuery('.trullify-mobile-menu-toggler').on('click', function() {
...
class trullify_Infinite_List {
    constructor( $obj = null, cfg = {} )  {
        ...
        // Hover Event
        this.$el.on('mouseenter', '.trullify-infinite-list-item', function() {
            ...
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.trullify-infinite-list-preview').children('video')[0].play();
                $preview.children('video')[0].play();
            }
        }).on('mouseleave', '.trullify-infinite-list-item', function() {
            _self.$el.removeClass('is-hovererd');
            let $preview = _self.$bg_wrap.children('[data-id="'+ this.getAttribute('data-id') +'"]');
            //$preview.removeClass('is-active');
            if ('video' === $preview.attr('data-type') ) {
                jQuery(this).children('.trullify-infinite-list-preview').children('video')[0].pause();
    ...
    init() {
        ...
        // DOM
        this.$list = this.$el.children('.trullify-infinite-list');
        this.$bg_wrap = jQuery('<div class="trullify-infinite-list-bgs"/>').appendTo(this.$el);
        this.$scroll = jQuery('<div class="trullify-infinite-scroll"/>').appendTo(this.$el);
        this.$cloned_list = jQuery('<div class="trullify-infinite-scroll-inner"/>').appendTo(this.$scroll);
    ...
    clone_items() {
        ...
        this.$list.children().each(function(i) {
            ...
            if ( _self.first_circle ) {
                let $preview = $cloned.children('.trullify-infinite-list-preview');
                ...
                if ( 'video' === type ) {
                    ...
                    <div class="trullifyl-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="video" data-id="${i}" style="background-image:url(${this_src})">
                        <video src="${this_src}" webkit-playsinline="true" playsinline="true" muted loop></video>
                    </div>`);
                } else {
                    $preview.css('background-image', 'url('+ this_src + ')');
                    _self.$bg_wrap.append(`<div class="trullifyl-bg-preview-item${(i === 0 ? ' is-active':'')}" data-type="image" data-id="${i}" style="background-image:url(${this_src})"/>`);
                }
            }
        });
...
class trullify_Marquee {
    constructor( $obj = null )  {
        ...
        this.$inner = this.$marquee.children('.trullify-marquee-inner');
        this.$content = this.$inner.children();
        this.width = this.$inner.find('.trullify-marquee-list').width();
    ...
    init() {
        this.$inner.wrapInner('<div class="trullify-marquee-inner-wrap"/>');
        this.$inner_cont = this.$inner.children('.trullify-marquee-inner-wrap');
...
class trullify_Masked {
    constructor( $obj = null, cfg = {} ) {
        ...
        // Options
        this.cfg = Object.assign({
            media_selector: '.trullify-masked-media',
            content_selector: '.trullify-masked-content',
            dbr: 24,
            ibr: 32
        }, cfg);
...
class trullify_LWP {
    constructor( $obj = null )  {
        ...
        this.$media = jQuery('<div class="trullify-lwp-media" data-appear="fade-right"/>').prependTo(this.$el);
        this.count = this.$list.children('.trullify-lwp-item').length;
        this.active = 0;
        this.prev = 1;

        this.$list.children('.trullify-lwp-item').each(function(i) {
